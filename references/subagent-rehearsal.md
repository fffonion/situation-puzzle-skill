...
- 区分必要因果边与画面细节。核心条件、时间顺序、动作机制、最终反应四项中已有三项得到支持，且当前假设能解释全部汤面异常时，优先提交完整假设，不再盘问姿势、具体物件形态、可替换动作或观察角度。
- 同一语义区域连续两次得到“是也不是”时停止同义改写：只定位一个会改变核心链的未确认变量；若没有这种变量，下一轮直接提交完整假设。已有完整因果链后，局部细节再次得到“是也不是”，最多再用两轮修正措辞并重新提交整条链。
- 第 35 轮起每轮先做闭合检查；已经能解释全部异常和最终反应时，下一问必须是完整假设。第 45 轮起禁止开启新支线或重复确认装饰细节；已有可闭合主线时，下一问必须提交当前最佳完整假设。
- 完整假设按最短因果顺序表达：隐藏条件／前史 → 关键动作 → 被误读的汤面事实 → 最终反应及原因。允许少量不改变机制的未确认措辞，禁止加入会改变身份、时间、动作机制或因果方向的新前提。
...
- [ ] 玩家提示词包含终局收敛规则：同一语义区域连续两次“是也不是”后停止同义改写；第 35／45 轮执行闭合检查；主线可闭合时提交完整因果链。
